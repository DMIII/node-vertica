...
Query = class Query extends EventEmitter {
  constructor(connection, sql, callback1) {
    super();
    ...
    if (this.callback) {
      return process.nextTick(() => {
        if (this.error) {
          ...
          return this.callback(null, new Resultset({
            fields: this.fields,
            rows: this.rows,
            status: this.status
          }));
  ...
  onCopyInResponse(msg) {
    ...
    dataHandler = (data, callback) => {
      return this.copyData(data, callback);
    };
    successHandler = (callback) => {
      return this.copyDone(callback);
    };
    failureHandler = (err, callback) => {
      return this.copyFail(err, callback);
    };
    try {
  ...
  _getCopyInHandler() {
    ...
    } else if (typeof this.copyInSource === 'string') { // copy from file
      fs = require('fs');
    ...
    } else if (this.copyInSource === process.stdin) { // copy from STDIN
